...
import { Script, type ScriptMeta } from "./script";
...
import type { LoadScriptMsg } from "../../common/messages";
...
declare let __scripterMonaco: {
	...
	patchBasenameOrAuthority(
		f?: (resource: monaco.Uri) => string | null | undefined,
	);
};
...
const varspaceFontFamily =
	"iaw-quattro-var, iaw-quattro, monospace, 'Inter var'";
...
export function ModelChangeFlagsString(flags: ModelChangeFlags): string {
	...
	if (flags & ModelChangeFlags.REMOVE_LINE) {
		s.push("REMOVE_LINE");
	}
	if (flags & ModelChangeFlags.LAST_LINE_INTACT) {
		s.push("LAST_LINE_INTACT");
	}
...
export class EditorState extends EventEmitter<EditorStateEvents> {
	...
	get currentModel(): EditorModel {
		return this._currentModel;
	}
	get currentScript(): Script {
		return this._currentScript;
	}
	get currentScriptId(): number {
		return this._currentScript ? this._currentScript.id : -1;
	}
	...
	disposeModel(script: Script) {
		...
		if (this._currentModel === model) {
			console.warn(
				`editor/disposeModel attempting to dispose active model (ignoring)`,
			);
			return;
		}
	...
	setCurrentScript(script: Script): EditorModel {
		...
		if (!script.isLoaded) {
			console.warn(
				"editor/setCurrentScript with not-yet-loaded script",
				script,
			);
		}
	...
	async openScript(script: Script): Promise<Script | null> {
		if (
			this._currentScript.guid == script.guid ||
			this._currentScriptId == script.id
		) {
			this.editor.focus();
			return this._currentScript;
	...
	async compileCurrentScript(): Promise<ScriptProgram> {
		...
		try {
			lockModel(model);
			let tsworkerForModel =
				await monaco.languages.typescript.getTypeScriptWorker();
			let tsclient = await tsworkerForModel(model.uri);
			result = (await tsclient.getEmitOutput(
				model.uri.toString(),
			)) as EmitOutput;
		} finally {
			unlockModel(model);
	...
	removeDecorations(ids: string[]) {
		...
		this.editorDecorationIds = this.editorDecorationIds.filter(
			(id) => !idset.has(id),
		);
		for (let id of ids) {
	...
	startObservingChanges() {
		...
		this.editor.onDidChangeModelContent(
			(e: monaco.editor.IModelContentChangedEvent) => {
					...
					if (c.text == "") {
						...
						if (endColumn == 1) {
							...
							if (endLine == startLine) {
								console.warn(
									"[scripter] unexpected condition: endLine == startLine",
								);
							}
						}
					} /*if (c.rangeLength == 0)*/ else {
				...
				// update zones
				this.viewZones.updateAfterEdit(
					startLine,
					startColumn,
					endLine,
					endColumn,
	...
	async monacoOpenCodeEditor(
		...
		if (editor !== this.editor) {
			dlog(
				"monacoOpenCodeEditor got unexpected editor object (!== this.editor)",
				editor,
			);
			return null;
		}
	...
	ensureTrailingNewline() {
		...
		const lastVisibleLineContent =
			this.currentModel.getLineContent(lastVisibleLineno);
	...
	adjustSelection() {
		...
		if (changed) {
			if (DEBUG_HIDDEN_AREAS) {
				dlog(
					"patch selection (DEBUG_HIDDEN_AREAS)",
	...
	initEditorEventHandlers() {
		...
		editor.addCommand(monaco.KeyCode.F1, (ctx: any) => {
			editor.trigger("", "editor.action.quickCommand", {});
		});
		...
		editor.addCommand(
			...
			(ctx: any) => {
				editor.trigger("", "editor.action.quickOutline", {});
			},
		);

		// go to definition
		// Note: shows "peek definition" instead when there are many defs.
		editor.addCommand(monaco.KeyCode.F12, (ctx: any) => {
			editor.trigger("", "editor.action.revealDefinition", {});
		});

		editor.addCommand(monaco.KeyCode.F11, (ctx: any) => {
			editor.trigger("", "editor.action.peekDefinition", {});
		});

		// cmd-alt-/ -- toggle block comment
		editor.addCommand(
			monaco.KeyMod.CtrlCmd | monaco.KeyMod.Alt | monaco.KeyCode.US_SLASH,
			(ctx: any) => {
	...
	initEditorModel(model: EditorModel) {
		...
		model.onDidChangeDecorations(async (ev) => {
			...
			try {
				...
				const filterRe =
					/['"]?return['"]? statement can only be used within a function body/i;
