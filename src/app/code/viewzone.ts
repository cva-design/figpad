...
import type { UIInputResponseMsg } from "../../common/messages";
...
function registerDOMMountCallback(
	editor: EditorState,
	el: HTMLElement,
	callback: () => void,
) {
	...
	let ent = editor[kViewZoneDOMObserver] as
		| [DomMountQueue, MutationObserver]
		| null;
...
export class ViewZone
	extends EventEmitter<ViewZoneEvents>
	implements monaco.editor.IViewZone
{
	...
	_updateSourceLineLen() {
		try {
			(this as any).sourceLineLen = this.editor.currentModel.getLineLength(
				this.sourceLine,
			);
		} catch (e) {
			console.warn(
...
export class PrintViewZone extends ViewZone {
	...
	constructor(
		...
		if (messageHtml != "") {
			...
			inlineButtonEl.addEventListener(
				"click",
				(ev) => {
					ev.stopPropagation();
					ev.preventDefault();
	...
	addAsCode() {
		...
		setTimeout(() => {
			editor.editor.setSelection(newSelection);
		}, 1);
	}
}
...
export class InputViewZone extends ViewZone {
	...
	enqueueResolver(resolver: InputResolver) {
		if (this.nextResolver) {
			console.warn(
				"[scripter] enqueueResolver while this.nextResolver != null",
			);
...
export type InputResolver = (
	msg: Omit<UIInputResponseMsg, "id" | "type">,
) => void;
